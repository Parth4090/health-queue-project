...
const DoctorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Check if doctor requires verification
        if (result.requiresVerification) {
          // Redirect to verification status page
          navigate('/verification-status', { 
            state: { 
              verificationId: result.user.verificationId,
              tempToken: result.user.tempToken 
            } 
          });
          return;
        }
        
        // Redirect to doctor dashboard for verified doctors
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl p-8 border border-white/20">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Sign in to manage your patient queues</p>
          </div>
